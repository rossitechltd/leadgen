...
def run(ctx: PipelineContext) -> StepResult:
    """
    Step 9: Move qualified leads to a dated Finalised sheet and clear Dynamic Lead Sheet.
    ...
    ctx.add_log("Step 9: finalizing qualified leads to dated sheet")

    result = service.run()
    if not result.ok:
        ctx.add_log(f"Step 9 failed: {result.message}")
        return StepResult(
    ...
    ctx.add_log(f"Step 9: {result.message}")
